...
public class Room implements Iterable<Player> {
    ...
    static Room newRoom(JsonObject jsonObject, Bot bot) {
        ...
        room.refreshRoomData(jsonObject, bot);
        return room;
    }
    ...
    void refreshRoomData(JsonObject jsonObject, Bot bot) {
        ...
        for (int ctr = 1; ctr < playersAsJson.length; ctr++) {
            ...
            if (player == null) {
                this.set(ctr, Player.newPlayer(playersAsJson[ctr], bot,this));
                continue;
            }
            if (player.userId.equals(playersAsJson[ctr].get("userId"))) {
                player.refreshPlayerData(playersAsJson[ctr], bot,this);
            } else {
                this.set(ctr, Player.newPlayer(playersAsJson[ctr], bot,this));
            }
        }
    ...
    final boolean isCurTurn(Player player) {
